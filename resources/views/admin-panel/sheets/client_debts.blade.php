...
@section('content')
    ...
    <div class="side-app">
        <div class="text-center mb-3">
            <h1 class="page-title mb-0 text-primary fs-30">مديونية العملاء </h1>
        </div>
            ...
            <div class="card">
                <div class="card-header mb-5">
                    ...
                    <div class="card-title" style=" font-size: 17px; font-weight: 500; ">
                        مديونات العملاء (التقرير الشهرى)
                    </div>
                </div>
                <table class="table table-striped">
                    ...
                    @foreach($debts as $userId => $debt)
                        <tr>
                            <td>{{$loop->iteration}}</td>
                            <td>{{$debt['user']['name']}} </td>
                            <td>{{$debt['user']['phone']}} </td>
                            <td>{{$debt['user']['contract_type'] == 'contact' ? 'تعاقد' : 'كاش'}} </td>
                            <td>{{$debt['total_debts']}} جنيه</td>
                            <td>
                                <button type="button" class="btn btn-teal"
                                        onclick="ExportExcel('{{ $userId }}')">
...
@push('scripts')
    ...
    <script>
        function ExportExcel(month) {
            ...
            var url = `{{ route('admin.sheet.download-client-debts') }}?user_id=${month}`;
        ...
        function ExportDailyExcel(day) {
            // Constructing the URL with month and doctorId parameters
            var url = `{{ route('admin.sheet.download-client-debts') }}?user_id=${day}`;
