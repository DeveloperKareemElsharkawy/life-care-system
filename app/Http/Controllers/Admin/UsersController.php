...
//        return view('admin-panel.users.show', compact('user', 'states', 'insuranceCompanies'));
    }
    ...
    public function create()
    {
        ...
        return view('admin-panel.users.create', compact('states', 'doctors', 'categories', 'insuranceCompanies'));
    }
    ...
    public function store(CreateUserRequest $request, UploadImageService $uploadImageService)
    {
        ...
        if ($request['image']) {
            ...
            $validated['image'] = $request->safe()->merge(['image' => $image]);
        }

        User::create($validated);

        return to_route('admin.contract-users.index')->with('successful_message', trans('admin/messages.users.created'));
    }
    ...
    public function edit($systemUserId)
    {
        ...
        return view('admin-panel.users.edit', compact('user', 'states', 'categories', 'doctors', 'insuranceCompanies'));
    }
    ...
    public function update(UpdateUserRequest $request, UploadImageService $uploadImageService)
    {
        ...
        return to_route('admin.contract-users.index')->with('successful_message', trans('admin/messages.users.updated'));
    }


    public function getDoctorExaminationPercentage(Request $request)
    {
        $doctorProfit =  DoctorProfitPercentage::query()->where('doctor_id', $request->doctor_id)->where('category_id', $request->category_id)->first();

        if ($doctorProfit && $doctorProfit->examination_profit_percentage){
            return $doctorProfit->examination_profit_percentage;
        }
