...
final class PaymentMethodsTable extends PowerGridComponent
{
    use ActionButton;

    //Messages informing success/error data is updated.
    public bool $showUpdateMessages = true;
    public array $PaymentMethodIdsForDeletion = [];
    ...
    *
    * @return  \Illuminate\Database\Eloquent\Builder<\App\Models\PaymentMethod>|null
    */
    ...
    */
    public function addColumns(): ?PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('created_at_formatted', function (PaymentMethod $model) {
                return Carbon::parse($model->created_at)->format('d/m/Y H:i:s');
            })
            ->addColumn('updated_at_formatted', function (PaymentMethod $model) {
     ...
     public function actions(): array
    {
       return [
           Button::add('edit')
               ...
               ->target('_self')
               ->route('admin.payment-methods.edit', ['payment_method_id' => 'id']),
           ...
           Button::add('destroy')
               ...
               ->target('_self')
               ->emit('ConfirmRemovePaymentMethod', ['PaymentMethodIds' => 'id']),
        ];
    }
   ...
   public function header(): array
     {
         ...
         return [
             Button::add('bulk-sold-out')
                 ->caption($CreatNewBtnIcon . __('admin/datatable.buttons.c_payment_method'))
             ...
             Button::add('bulk-sold-out')
                 ...
                 ->target('_self')
                 ->emit('ConfirmRemovePaymentMethod', []),
         ];
     }
    ...
    */
    ...
    protected function getListeners()
    {
        return array_merge(
            ...
            'deleteConfirmed' => 'deletePaymentMethod',

        ]);
    }

    public function ConfirmRemovePaymentMethod($data)
    {
        if (count($this->checkboxValues) == 0 && !isset($data['PaymentMethodIds'])) {
            $this->dispatchBrowserEvent('NoSelectedItemsAlert', ['message' => 'You must select at least one item!']);
            return;
        }

        $systemUsersIds = isset($data['PaymentMethodIds']) ? [$data['PaymentMethodIds']] : $this->checkboxValues;

        $this->PaymentMethodIdsForDeletion = $systemUsersIds;
    ...
    public function deletePaymentMethod()
    {
        $ids = array_map(function ($value) {
            return intval($value);
        }, $this->PaymentMethodIdsForDeletion);
    ...
    */
     ...
     /**
     * PowerGrid PaymentMethod Action Rules.
     *
    ...
    public function actionRules(): array
    {
       return [
           ...
           //Hide button edit for ID 1
            Rule::button('edit')
                ->when(fn($payment-method) => $payment-method->id === 1)
                ->hide(),
        ];
    }
    */
    ...
    */
     ...
     /**
     * PowerGrid PaymentMethod Update.
     *
