...
final class VehicleTypeTable extends PowerGridComponent
{
    use ActionButton;
    ...
    //Messages informing success/error data is updated.
    public array $vehiclesTypesIdsForDeletion = [];
    ...
    protected function getListeners(): array
    {
        return array_merge(
            parent::getListeners(), [
            'alertEvent',
            'ConfirmRemoveVehiclesTypes',
            'deleteConfirmed' => 'deleteSystemUsers'

        ]);
    }

    public function ConfirmRemoveVehiclesTypes($data)
    {

        if (count($this->checkboxValues) == 0 && !isset($data['vehicles_type_id'])) {
            $this->dispatchBrowserEvent('NoSelectedItemsAlert', ['message' => 'You must select at least one item!']);
            return;
        }

        $systemUsersIds = isset($data['vehicles_type_id']) ? [$data['vehicles_type_id']] : $this->checkboxValues;

        $this->vehiclesTypesIdsForDeletion = $systemUsersIds;
    ...
    public function deleteSystemUsers()
    {
        $ids = array_map(function ($value) {
            return intval($value);
        }, $this->vehiclesTypesIdsForDeletion);
        VehicleType::query()->whereIn('id', $ids)->delete();
        $this->vehiclesTypesIdsForDeletion = [];
    }
    ...
    */
    public function addColumns(): ?PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ->addColumn('name')
            ->addColumn('created_at_formatted', function (VehicleType $model) {
                return Carbon::parse($model->created_at)->format('d/m/Y H:i:s');
            })
            ->addColumn('updated_at_formatted', function (VehicleType $model) {
    ...
    */
    ...
    /**
     * PowerGrid VehicleType Action Buttons.
     *
    ...
    public function actions(): array
    {
        return [
            Button::add('edit')
                ...
                ->target('_self')
                ->route('admin.vehicles.types.edit', ['vehicles_type_id' => 'id']),
            ...
            Button::add('destroy')
                ...
                ->target('_self')
                ->emit('ConfirmRemoveVehiclesTypes', ['vehicles_type_id' => 'id']),
        ];
    }
    ...
    public function header(): array
    {
        ...
        return [
            Button::add('bulk-sold-out')
                ...
                ->target('_self')
                ->route('admin.vehicles.types.create', []),
    ...
    */
    ...
    /**
     * PowerGrid VehicleType Action Rules.
     *
    ...
    public function actionRules(): array
    {
       return [
           ...
           //Hide button edit for ID 1
            Rule::button('edit')
                ->when(fn($vehicle-type) => $vehicle-type->id === 1)
                ->hide(),
        ];
    }
    */
    ...
    */
    ...
    {
        try {
            $updated = VehicleType::query()->findOrFail($data['id'])
                ->update([
