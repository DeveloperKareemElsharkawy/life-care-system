...
final class InsuranceCompanyTable extends PowerGridComponent
{
    use ActionButton;

    //Messages informing success/error data is updated.
    public bool $showUpdateMessages = true;
    public array $InsuranceCompanyIdsForDeletion = [];
    ...
    */
     ...
     *
     * @return  \Illuminate\Database\Eloquent\Builder<\App\Models\InsuranceCompany>|null
     */
    ...
    */
    public function addColumns(): ?PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ->addColumn('id')
            ->addColumn('name')
            ->addColumn('image', function (InsuranceCompany $model) {
                return '<span class="avatar avatar-xxl bradius" style="background-image: url(' . $model->imageUrl . ')"></span>';
            })
            ->addColumn('created_at')
            ->addColumn('created_at_formatted', function (InsuranceCompany $model) {
    ...
    */
    ...
    public function actions(): array
    {
        return [
            ...
            Button::add('edit')
                ->caption('<i class="fa fa-money"  title="" data-bs-original-title="fe fe-edit-3" aria-label="fe fe-edit-3"></i>')
                ->class('btn btn-icon btn-secondary')
                ->target('_self')
                ->route('admin.insurance-company-categories.index', ['insurance_company_id' => 'id']),

            Button::add('edit')
                ->caption('<i class="fe fe-edit"  title="" data-bs-original-title="fe fe-edit-3" aria-label="fe fe-edit-3"></i>')
                ->class('btn btn-icon btn-secondary')
                ->target('_self')
                ->route('admin.insurance-companies.edit', ['insurance_company_id' => 'id']),
            ...
            Button::add('destroy')
                ...
                ->target('_self')
                ->emit('ConfirmRemoveInsuranceCompany', ['InsuranceCompanyIds' => 'id']),
        ];
    }
    ...
    public function header(): array
    {
        ...
        return [
            Button::add('bulk-sold-out')
                ->caption($CreatNewBtnIcon . __('admin/datatable.buttons.c_insurance_company'))
                ->class('btn btn-success  btn-svgs btn-svg-white datatable-delete-btn')
                ->target('_self')
                ->route('admin.insurance-companies.create', []),
            ...
            Button::add('bulk-sold-out')
                ...
                ->target('_self')
                ->emit('ConfirmRemoveInsuranceCompany', []),
        ];
    }
    ...
    */
    ...
    protected function getListeners()
    {
        return array_merge(
            parent::getListeners(), [
            'alertEvent',
            'ConfirmRemoveInsuranceCompany',
            'deleteConfirmed' => 'deleteInsuranceCompany',

        ]);
    }

    public function ConfirmRemoveInsuranceCompany($data)
    {
        if (count($this->checkboxValues) == 0 && !isset($data['InsuranceCompanyIds'])) {
            $this->dispatchBrowserEvent('NoSelectedItemsAlert', ['message' => 'You must select at least one item!']);
            return;
        }

        $systemUsersIds = isset($data['InsuranceCompanyIds']) ? [$data['InsuranceCompanyIds']] : $this->checkboxValues;

        $this->InsuranceCompanyIdsForDeletion = $systemUsersIds;

        $this->dispatchBrowserEvent('show-delete-confirmation', ['message' => 'You have selected IDs: ']);
    }

    public function deleteInsuranceCompany()
    {
        $ids = array_map(function ($value) {
            return intval($value);
        }, $this->InsuranceCompanyIdsForDeletion);
        InsuranceCompany::query()->whereIn('id', $ids)->delete();
        $this->InsuranceCompanyIdsForDeletion = [];
    }
    ...
    */
    ...
    /**
     * PowerGrid InsuranceCompany Action Rules.
     *
    ...
    public function actionRules(): array
    {
       return [
           ...
           //Hide button edit for ID 1
            Rule::button('edit')
                ->when(fn($insurance-company) => $insurance-company->id === 1)
                ->hide(),
        ];
    }
    */
    ...
    */
    ...
    /**
     * PowerGrid InsuranceCompany Update.
     *
    ...
    {
       try {
           $updated = InsuranceCompany::query()
                ->update([
