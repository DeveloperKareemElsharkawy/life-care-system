...
final class StatesTable extends PowerGridComponent
{
    ...
    public array $StateIdsForDeletion = [];
    ...
    *
    * @return  \Illuminate\Database\Eloquent\Builder<\App\Models\State>|null
    */
    ...
    */
    public function addColumns(): ?PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('created_at_formatted', function (State $model) {
                return Carbon::parse($model->created_at)->format('d/m/Y H:i:s');
            })
            ->addColumn('updated_at_formatted', function (State $model) {
     ...
     public function actions(): array
    {
       return [
           Button::add('edit')
               ...
               ->target('_self')
               ->route('admin.states.edit', ['state_id' => 'id']),
           ...
           Button::add('destroy')
               ...
               ->target('_self')
               ->emit('ConfirmRemoveState', ['StateIds' => 'id']),
        ];
    }
   ...
   public function header(): array
     {
         ...
         return [
             Button::add('bulk-sold-out')
                 ->caption($CreatNewBtnIcon . __('admin/datatable.buttons.c_state'))
    ...
    */
    ...
    protected function getListeners()
    {
        return array_merge(
            ...
            'alertEvent',
            'ConfirmRemoveState',
            'deleteConfirmed' => 'deleteState',

        ]);
    }

    public function ConfirmRemoveState($data)
    {
        if (count($this->checkboxValues) == 0 && !isset($data['StateIds'])) {
            $this->dispatchBrowserEvent('NoSelectedItemsAlert', ['message' => 'You must select at least one item!']);
            return;
        }

        $systemUsersIds = isset($data['StateIds']) ? [$data['StateIds']] : $this->checkboxValues;

        $this->StateIdsForDeletion = $systemUsersIds;
