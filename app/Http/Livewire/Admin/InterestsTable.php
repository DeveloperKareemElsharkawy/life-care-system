...
final class InterestsTable extends PowerGridComponent
{
    use ActionButton;

    //Messages informing success/error data is updated.
    public bool $showUpdateMessages = true;
    public array $InterestIdsForDeletion = [];
    ...
    *
    * @return  \Illuminate\Database\Eloquent\Builder<\App\Models\Interest>|null
    */
    ...
    */
    public function addColumns(): ?PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('created_at_formatted', function (Interest $model) {
                return Carbon::parse($model->created_at)->format('d/m/Y H:i:s');
            })
            ->addColumn('updated_at_formatted', function (Interest $model) {
     ...
     public function actions(): array
    {
       return [
           Button::add('edit')
               ...
               ->target('_self')
               ->route('admin.interests.edit', ['interest_id' => 'id']),
           ...
           Button::add('destroy')
               ...
               ->target('_self')
               ->emit('ConfirmRemoveInterest', ['InterestIds' => 'id']),
        ];
    }
   ...
   public function header(): array
     {
         ...
         return [
             Button::add('bulk-sold-out')
                 ->caption($CreatNewBtnIcon . __('admin/datatable.buttons.c_interest'))
                 ->class('btn btn-success  btn-svgs btn-svg-white datatable-delete-btn')
                 ->target('_self')
                 ->route('admin.interests.create', []),
    ...
    */
    ...
    protected function getListeners()
    {
        return array_merge(
            ...
            'deleteConfirmed' => 'deleteInterest',

        ]);
    }

    public function ConfirmRemoveInterest($data)
    {
        if (count($this->checkboxValues) == 0 && !isset($data['InterestIds'])) {
            $this->dispatchBrowserEvent('NoSelectedItemsAlert', ['message' => 'You must select at least one item!']);
            return;
        }

        $systemUsersIds = isset($data['InterestIds']) ? [$data['InterestIds']] : $this->checkboxValues;

        $this->InterestIdsForDeletion = $systemUsersIds;
    ...
    public function deleteInterest()
    {
        $ids = array_map(function ($value) {
            return intval($value);
        }, $this->InterestIdsForDeletion);
        Interest::query()->whereIn('id', $ids)->delete();
        $this->InterestIdsForDeletion = [];
    }
    ...
    */
     ...
     /**
     * PowerGrid Interest Action Rules.
     *
    ...
    public function actionRules(): array
    {
       return [
           ...
           //Hide button edit for ID 1
            Rule::button('edit')
                ->when(fn($interest) => $interest->id === 1)
                ->hide(),
        ];
    }
    */
