...
final class SliderTable extends PowerGridComponent
{
    ...
    public array $SliderIdsForDeletion = [];
    ...
    */
    public function addColumns(): ?PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('created_at_formatted', function (Slider $model) {
    ...
    */
    ...
    public function columns(): array
    {
        return [
            ...
            Column::add()
                ->title(trans('admin/datatable.title'))
                ->field('title')
                ->searchable()
                ->makeInputText('title')
                ->sortable(),
            ...
            Column::add()
                ->title(trans('admin/datatable.image'))
                ->field('image'),
    ...
    public function actions(): array
    {
        return [
            Button::add('edit')
                ...
                ->target('_self')
                ->route('admin.slider.edit', ['slider_id' => 'id']),
            ...
            Button::add('destroy')
                ...
                ->target('_self')
                ->emit('ConfirmRemoveSlider', ['SliderIds' => 'id']),
        ];
    }
    ...
    public function header(): array
    {
        ...
        return [
            Button::add('bulk-sold-out')
                ->caption($CreatNewBtnIcon . __('admin/datatable.buttons.c_slider'))
    ...
    */
    ...
    protected function getListeners()
    {
        return array_merge(
            ...
            'alertEvent',
            'ConfirmRemoveSlider',
            'deleteConfirmed' => 'deleteSlider',

        ]);
    }

    public function ConfirmRemoveSlider($data)
    {
        if (count($this->checkboxValues) == 0 && !isset($data['SliderIds'])) {
            $this->dispatchBrowserEvent('NoSelectedItemsAlert', ['message' => 'You must select at least one item!']);
            return;
        }

        $systemUsersIds = isset($data['SliderIds']) ? [$data['SliderIds']] : $this->checkboxValues;

        $this->SliderIdsForDeletion = $systemUsersIds;
    ...
    public function deleteSlider()
    {
        $ids = array_map(function ($value) {
            return intval($value);
        }, $this->SliderIdsForDeletion);
        Slider::query()->whereIn('id', $ids)->delete();
        $this->SliderIdsForDeletion = [];
    }
    ...
    */
    ...
    {
        try {
            $updated = Slider::query()->findOrFail($data['id'])
                ->update([
