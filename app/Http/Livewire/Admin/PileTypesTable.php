...
final class PileTypesTable extends PowerGridComponent
{
    use ActionButton;

    //Messages informing success/error data is updated.
    public bool $showUpdateMessages = true;
    public array $PileTypeIdsForDeletion = [];
    ...
    *
    * @return  \Illuminate\Database\Eloquent\Builder<\App\Models\PileType>|null
    */
    public function datasource(): ?Builder
    {
        return PileType::query();
    }
    ...
    */
    public function addColumns(): ?PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('created_at_formatted', function(PileType $model) {
     ...
     public function actions(): array
    {
       return [
           Button::add('edit')
               ...
               ->target('_self')
               ->route('admin.pile-types.edit', ['pile_type_id' => 'id']),
           ...
           Button::add('destroy')
               ...
               ->target('_self')
               ->emit('ConfirmRemovePileType', ['PileTypeIds' => 'id']),
        ];
    }
   ...
   public function header(): array
     {
         ...
         return [
             Button::add('bulk-sold-out')
                 ->caption($CreatNewBtnIcon . __('admin/datatable.buttons.c_pile_type'))
                 ->class('btn btn-success  btn-svgs btn-svg-white datatable-delete-btn')
                 ->target('_self')
                 ->route('admin.pile-types.create', []),
    ...
    */
    ...
    protected function getListeners()
    {
        return array_merge(
            parent::getListeners(), [
            'alertEvent',
            'ConfirmRemovePileType',
            'deleteConfirmed' => 'deletePileType',

        ]);
    }

    public function ConfirmRemovePileType($data)
    {
        if (count($this->checkboxValues) == 0 && !isset($data['PileTypeIds'])) {
            $this->dispatchBrowserEvent('NoSelectedItemsAlert', ['message' => 'You must select at least one item!']);
            return;
        }

        $systemUsersIds = isset($data['PileTypeIds']) ? [$data['PileTypeIds']] : $this->checkboxValues;

        $this->PileTypeIdsForDeletion = $systemUsersIds;

        $this->dispatchBrowserEvent('show-delete-confirmation', ['message' => 'You have selected IDs: ']);
    }

    public function deletePileType()
    {
        $ids = array_map(function ($value) {
            return intval($value);
        }, $this->PileTypeIdsForDeletion);
        PileType::query()->whereIn('id', $ids)->delete();
        $this->PileTypeIdsForDeletion = [];
    }
    ...
    */
     ...
     /**
     * PowerGrid PileType Action Rules.
     *
    ...
    public function actionRules(): array
    {
       return [
           ...
           //Hide button edit for ID 1
            Rule::button('edit')
                ->when(fn($pile-type) => $pile-type->id === 1)
                ->hide(),
        ];
    }
    */
