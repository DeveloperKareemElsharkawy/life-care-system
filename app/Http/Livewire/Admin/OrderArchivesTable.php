...
final class OrderArchivesTable extends PowerGridComponent
{
    use ActionButton;

    //Messages informing success/error data is updated.
    public bool $showUpdateMessages = true;
    public array $OrderArchiveIdsForDeletion = [];
    ...
    *
    * @return  \Illuminate\Database\Eloquent\Builder<\App\Models\OrderArchive>|null
    */
    public function datasource(): ?Builder
    {
        return OrderArchive::query()->where('order_id', $this->order_id);
    }
    ...
    */
    public function addColumns(): ?PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('created_at_formatted', function (OrderArchive $model) {
     ...
     public function actions(): array
    {
       return [
           Button::add('edit')
               ...
               ->target('_self')
               ->route('admin.orders_archives.edit', ['orders_archive_id' => 'id' , 'order_id' => $this->order_id]),
           ...
           Button::add('destroy')
               ...
               ->target('_self')
               ->emit('ConfirmRemoveOrderArchive', ['OrderArchiveIds' => 'id']),
        ];
    }
   ...
   public function header(): array
     {
         ...
         return [
             Button::add('bulk-sold-out')
                 ->caption($CreatNewBtnIcon . __('admin/datatable.buttons.c_order_archive'))
                 ->class('btn btn-success  btn-svgs btn-svg-white datatable-delete-btn')
                 ->target('_self')
                 ->route('admin.orders_archives.create', ['order_id' => $this->order_id]),
             ...
             Button::add('bulk-sold-out')
                 ...
                 ->target('_self')
                 ->emit('ConfirmRemoveOrderArchive', []),
         ];
     }
    ...
    */
    ...
    protected function getListeners()
    {
        return array_merge(
            parent::getListeners(), [
            'alertEvent',
            'ConfirmRemoveOrderArchive',
            'deleteConfirmed' => 'deleteOrderArchive',

        ]);
    }

    public function ConfirmRemoveOrderArchive($data)
    {
        if (count($this->checkboxValues) == 0 && !isset($data['OrderArchiveIds'])) {
            $this->dispatchBrowserEvent('NoSelectedItemsAlert', ['message' => 'You must select at least one item!']);
            return;
        }

        $systemUsersIds = isset($data['OrderArchiveIds']) ? [$data['OrderArchiveIds']] : $this->checkboxValues;

        $this->OrderArchiveIdsForDeletion = $systemUsersIds;

        $this->dispatchBrowserEvent('show-delete-confirmation', ['message' => 'You have selected IDs: ']);
    }

    public function deleteOrderArchive()
    {
        $ids = array_map(function ($value) {
            return intval($value);
        }, $this->OrderArchiveIdsForDeletion);
        OrderArchive::query()->whereIn('id', $ids)->delete();
        $this->OrderArchiveIdsForDeletion = [];
    }
    ...
    */
     ...
     /**
     * PowerGrid OrderArchive Action Rules.
     *
    ...
    public function actionRules(): array
    {
       return [
           ...
           //Hide button edit for ID 1
            Rule::button('edit')
                ->when(fn($order-archive) => $order-archive->id === 1)
                ->hide(),
        ];
    }
    */
