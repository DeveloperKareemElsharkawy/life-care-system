...
final class SpecializationsTable extends PowerGridComponent
{
    use ActionButton;

    //Messages informing success/error data is updated.
    public bool $showUpdateMessages = true;
    public array $specializationIdsForDeletion = [];
    ...
    *
    * @return  \Illuminate\Database\Eloquent\Builder<\App\Models\specialization>|null
    */
    ...
    */
    public function addColumns(): ?PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('created_at_formatted', function(specialization $model) {
     ...
     public function actions(): array
    {
       return [
           Button::add('edit')
               ...
               ->target('_self')
               ->route('admin.specializations.edit', ['specialization_id' => 'id']),
           ...
           Button::add('destroy')
               ...
               ->target('_self')
               ->emit('ConfirmRemovespecialization', ['specializationIds' => 'id']),
        ];
    }
   ...
   public function header(): array
     {
         ...
         return [
             Button::add('bulk-sold-out')
                 ->caption($CreatNewBtnIcon . __('admin/datatable.buttons.c_specialization'))
             ...
             Button::add('bulk-sold-out')
                 ...
                 ->target('_self')
                 ->emit('ConfirmRemovespecialization', []),
         ];
     }
    ...
    */
    ...
    protected function getListeners()
    {
        return array_merge(
            parent::getListeners(), [
            'alertEvent',
            'ConfirmRemovespecialization',
            'deleteConfirmed' => 'deletespecialization',

        ]);
    }

    public function ConfirmRemovespecialization($data)
    {
        if (count($this->checkboxValues) == 0 && !isset($data['specializationIds'])) {
            $this->dispatchBrowserEvent('NoSelectedItemsAlert', ['message' => 'You must select at least one item!']);
            return;
        }

        $systemUsersIds = isset($data['specializationIds']) ? [$data['specializationIds']] : $this->checkboxValues;

        $this->specializationIdsForDeletion = $systemUsersIds;
    ...
    public function deletespecialization()
    {
        $ids = array_map(function ($value) {
            return intval($value);
        }, $this->specializationIdsForDeletion);
        specialization::query()->whereIn('id', $ids)->delete();
        $this->specializationIdsForDeletion = [];
    }
    ...
    */
     ...
     /**
     * PowerGrid specialization Update.
     *
