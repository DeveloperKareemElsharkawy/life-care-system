...
final class DriverTable extends PowerGridComponent
{
    ...
    //Messages informing success/error data is updated.
    public array $driversIdsForDeletion = [];
    ...
    protected function getListeners(): array
    {
        return array_merge(
            parent::getListeners(), [
            'alertEvent',
            'ConfirmRemoveDrivers',
            'deleteConfirmed' => 'deleteDrivers'

        ]);
    }

    public function ConfirmRemoveDrivers($data)
    {

        if (count($this->checkboxValues) == 0 && !isset($data['driver_id'])) {
            $this->dispatchBrowserEvent('NoSelectedItemsAlert', ['message' => 'You must select at least one item!']);
            return;
        }

        $systemUsersIds = isset($data['driver_id']) ? [$data['driver_id']] : $this->checkboxValues;

        $this->driversIdsForDeletion = $systemUsersIds;
    ...
    public function deleteDrivers()
    {
        $ids = array_map(function ($value) {
            return intval($value);
        }, $this->driversIdsForDeletion);
        Driver::query()->whereIn('id', $ids)->delete();
        $this->driversIdsForDeletion = [];
    }
    ...
    */
    public function addColumns(): ?PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ->addColumn('name')
            ->addColumn('image', function (Driver $model) {
    ...
    public function actions(): array
    {
        return [
            Button::add('edit')
                ...
                ->target('_self')
                ->route('admin.drivers.edit', ['driver_id' => 'id']),
            ...
            Button::add('destroy')
                ...
                ->target('_self')
                ->emit('ConfirmRemoveDrivers', ['driver_id' => 'id']),
        ];
    }
    ...
    public function header(): array
    {
        ...
        return [
            Button::add('bulk-sold-out')
                ->caption($CreatNewBtnIcon . __('admin/datatable.buttons.c_driver'))
