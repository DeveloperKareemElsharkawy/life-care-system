...
final class SchoolTable extends PowerGridComponent
{
    ...
    public array $SchoolIdsForDeletion = [];
    ...
    */
     ...
     *
     * @return  \Illuminate\Database\Eloquent\Builder<\App\Models\School>|null
     */
    ...
    */
    public function addColumns(): ?PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('name_en')
            ->addColumn('status')
            ->addColumn('created_at_formatted', function (School $model) {
                return Carbon::parse($model->created_at)->format('d/m/Y H:i:s');
            })
            ->addColumn('updated_at_formatted', function (School $model) {
    ...
    public function actions(): array
    {
        return [
            Button::add('edit')
                ...
                ->target('_self')
                ->route('admin.schools.edit', ['school_id' => 'id']),
            ...
            Button::add('destroy')
                ...
                ->target('_self')
                ->emit('ConfirmRemoveSchool', ['SchoolIds' => 'id']),
        ];
    }
    ...
    public function header(): array
    {
        ...
        return [
            Button::add('bulk-sold-out')
                ->caption($CreatNewBtnIcon . __('admin/datatable.buttons.c_school'))
    ...
    */
    ...
    protected function getListeners()
    {
        return array_merge(
            ...
            'alertEvent',
            'ConfirmRemoveSchool',
            'deleteConfirmed' => 'deleteSchool',

        ]);
    }

    public function ConfirmRemoveSchool($data)
    {
        if (count($this->checkboxValues) == 0 && !isset($data['SchoolIds'])) {
            $this->dispatchBrowserEvent('NoSelectedItemsAlert', ['message' => 'You must select at least one item!']);
            return;
        }

        $systemUsersIds = isset($data['SchoolIds']) ? [$data['SchoolIds']] : $this->checkboxValues;

        $this->SchoolIdsForDeletion = $systemUsersIds;
    ...
    public function deleteSchool()
    {
        $ids = array_map(function ($value) {
            return intval($value);
        }, $this->SchoolIdsForDeletion);
        School::query()->whereIn('id', $ids)->delete();
        $this->SchoolIdsForDeletion = [];
    }
