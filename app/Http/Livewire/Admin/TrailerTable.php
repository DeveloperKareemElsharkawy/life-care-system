...
final class TrailerTable extends PowerGridComponent
{
    use ActionButton;
    ...
    public array $trailersIdsForDeletion = [];
    ...
    protected function getListeners(): array
    {
        return array_merge(
            parent::getListeners(), [
            'alertEvent',
            'ConfirmRemoveTrailers',
            'deleteConfirmed' => 'deleteVehicles'

        ]);
    }

    public function ConfirmRemoveTrailers($data)
    {
        if (count($this->checkboxValues) == 0 && !isset($data['trailer_id'])) {
            $this->dispatchBrowserEvent('NoSelectedItemsAlert', ['message' => 'You must select at least one item!']);
            return;
        }

        $systemUsersIds = isset($data['trailer_id']) ? [$data['trailer_id']] : $this->checkboxValues;

        $this->trailersIdsForDeletion = $systemUsersIds;
    ...
    public function deleteVehicles()
    {
        $ids = array_map(function ($value) {
            return intval($value);
        }, $this->trailersIdsForDeletion);
        Trailer::query()->whereIn('id', $ids)->delete();
        $this->trailersIdsForDeletion = [];
    }
    ...
    */
    ...
    {
        return Trailer::query()->with('owners_percentage.owner', 'owners', 'type');
    }
    ...
    */
    ...
    {
        return [
            'owners' => [ // relationship on dishes model
    ...
    */
    public function addColumns(): ?PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ->addColumn('name')
            ->addColumn('image', function (Trailer $model) {
                return '<span class="avatar avatar-xxl bradius" style="background-image: url(' . $model->imageUrl . ')"></span>';
            })
            ->addColumn('vehicle_types_name', function (Trailer $model) {
                return $model->type ? $model->type->name : '';
            })
            ->addColumn('owners', function (Trailer $model) {
                $tags = '';
                foreach ($model->owners_percentage as $ownersPercentage) {
                    $tags .= "<span class='tag'>" . $ownersPercentage->owner->name . ' => ' . $ownersPercentage->percentage . '%' . " </span> ";

                }

                return $tags;
            })
            ->addColumn('created_at_formatted', function (Trailer $model) {
    ...
    */
    ...
    public function columns(): array
    {
        return [
            ...
            Column::add()
                ->title(trans('admin/datatable.vehicles_type'))
                ->field('vehicle_types_name')
                ->sortable()
                ->searchable()
                ->headerAttribute('datatable-header-txt', '')
                ->bodyAttribute('datatable-body-txt', '')
                ->makeInputSelect(VehicleType::all(), 'name', 'vehicle_type_id'),


            Column::add()
                ->title(trans('admin/datatable.vehicles_owner'))
                ->field('owners', 'owners.name')
                ->sortable()
                ->searchable()
                ->headerAttribute('datatable-header-txt', '')
                ->bodyAttribute('datatable-body-txt', '')
                ->makeInputSelect(Owner::query()->select('name')->distinct()->get(), 'name', 'name', ['class' => 'inline_search']),
    ...
    */
    ...
    /**
     * PowerGrid Trailer Action Buttons.
     *
    ...
    public function actions(): array
    {
        return [
            Button::add('edit')
                ...
                ->target('_self')
                ->route('admin.trailers.edit', ['trailer_id' => 'id']),
            ...
            Button::add('destroy')
                ...
                ->target('_self')
                ->emit('ConfirmRemoveTrailers', ['trailer_id' => 'id']),
        ];
    }
    ...
    public function header(): array
    {
        ...
        return [
            Button::add('bulk-sold-out')
                ->caption($CreatNewBtnIcon . __('admin/datatable.buttons.c_vehicle'))
                ->class('btn btn-success  btn-svgs btn-svg-white datatable-delete-btn')
                ->target('_self')
                ->route('admin.trailers.create', []),
    ...
    */
    ...
    /**
     * PowerGrid Trailer Action Rules.
     *
    ...
    */
    ...
    {
        try {
            $updated = Trailer::query()->findOrFail($data['id'])
                ->update([
