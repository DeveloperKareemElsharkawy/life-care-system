...
final class VehicleOwnerTable extends PowerGridComponent
{
    use ActionButton;
    ...
    //Messages informing success/error data is updated.
    public array $vehiclesOwnersIdsForDeletion = [];
    ...
    protected function getListeners(): array
    {
        return array_merge(
            parent::getListeners(), [
            'alertEvent',
            'ConfirmRemoveVehiclesOwners',
            'deleteConfirmed' => 'deleteSystemUsers'

        ]);
    }

    public function ConfirmRemoveVehiclesOwners($data)
    {

        if (count($this->checkboxValues) == 0 && !isset($data['vehicles_owner_id'])) {
            $this->dispatchBrowserEvent('NoSelectedItemsAlert', ['message' => 'You must select at least one item!']);
            return;
        }

        $systemUsersIds = isset($data['vehicles_owner_id']) ? [$data['vehicles_owner_id']] : $this->checkboxValues;

        $this->vehiclesOwnersIdsForDeletion = $systemUsersIds;
    ...
    public function deleteSystemUsers()
    {
        $ids = array_map(function ($value) {
            return intval($value);
        }, $this->vehiclesOwnersIdsForDeletion);
        Owner::query()->whereIn('id', $ids)->delete();
        $this->vehiclesOwnersIdsForDeletion = [];
    }
    ...
    public function actions(): array
    {
        return [
            Button::add('edit')
                ...
                ->target('_self')
                ->route('admin.vehicles.owners.edit', ['vehicles_owner_id' => 'id']),
            ...
            Button::add('destroy')
                ...
                ->target('_self')
                ->emit('ConfirmRemoveVehiclesOwners', ['vehicles_owner_id' => 'id']),
        ];
    }
    ...
    public function header(): array
    {
        ...
        return [
            Button::add('bulk-sold-out')
                ->caption($CreatNewBtnIcon . __('admin/datatable.buttons.c_owner'))
                ->class('btn btn-success  btn-svgs btn-svg-white datatable-delete-btn')
                ->target('_self')
                ->route('admin.vehicles.owners.create', []),
    ...
    */
    ...
    /**
     * PowerGrid VehiclesOwner Action Rules.
     *
    ...
    public function actionRules(): array
    {
       return [
           ...
           //Hide button edit for ID 1
            Rule::button('edit')
                ->when(fn($vehicles-owner) => $vehicles-owner->id === 1)
                ->hide(),
        ];
    }
    */
    ...
    */
    ...
    {
        try {
            $updated = Owner::query()->findOrFail($data['id'])
                ->update([
