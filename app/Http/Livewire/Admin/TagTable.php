...
final class TagTable extends PowerGridComponent
{
    ...
    public array $tagsIdsForDeletion = [];
    ...
    protected function getListeners()
    {
        return array_merge(
            ...
            'alertEvent',
            'ConfirmRemoveTags',
            'deleteConfirmed' => 'deleteTags'

        ]);
    }

    public function ConfirmRemoveTags($data)
    {
        if (count($this->checkboxValues) == 0 && !isset($data['tags_ids'])) {
            $this->dispatchBrowserEvent('NoSelectedItemsAlert', ['message' => 'You must select at least one item!']);
            return;
        }

        $systemUsersIds = isset($data['tags_ids']) ? [$data['tags_ids']] : $this->checkboxValues;

        $this->tagsIdsForDeletion = $systemUsersIds;
    ...
    *
    * @return  \Illuminate\Database\Eloquent\Builder<\App\Models\Tag>|null
    */
    ...
    public function actions(): array
    {
        return [
            Button::add('edit')
                ...
                ->target('_self')
                ->route('admin.tags.edit', ['tag_id' => 'id']),
            ...
            Button::add('destroy')
                ...
                ->target('_self')
                ->emit('ConfirmRemoveTags', ['tags_ids' => 'id']),
        ];
    }
    ...
    public function header(): array
    {
        ...
        return [
            Button::add('bulk-sold-out')
                ->caption($CreatNewBtnIcon . __('admin/datatable.buttons.c_tag'))
