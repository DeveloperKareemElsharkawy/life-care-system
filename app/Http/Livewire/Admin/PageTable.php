...
final class PageTable extends PowerGridComponent
{
    ...
    public array $PageIdsForDeletion = [];
...
//            Column::add()
//                ->title(trans('admin/datatable.slug'))
//                ->field('slug')
//                ->searchable()
//                ->makeInputText('slug')
//                ->sortable(),
    ...
    public function actions(): array
    {
        return [
            Button::add('edit')
                ...
                ->target('_self')
                ->route('admin.pages.edit', ['page_id' => 'id']),
            ...
            Button::add('destroy')
                ...
                ->target('_self')
                ->emit('ConfirmRemovePage', ['PageIds' => 'id']),
        ];
    }
    ...
    */
    ...
    public function ConfirmRemovePage($data)
    {
        if (count($this->checkboxValues) == 0 && !isset($data['PageIds'])) {
            $this->dispatchBrowserEvent('NoSelectedItemsAlert', ['message' => 'You must select at least one item!']);
            return;
        }

        $systemUsersIds = isset($data['PageIds']) ? [$data['PageIds']] : $this->checkboxValues;

        $this->PageIdsForDeletion = $systemUsersIds;
    ...
    public function deletePage()
    {
        $ids = array_map(function ($value) {
            return intval($value);
        }, $this->PageIdsForDeletion);
        Page::query()->whereIn('id', $ids)->delete();
        $this->PageIdsForDeletion = [];
    }
